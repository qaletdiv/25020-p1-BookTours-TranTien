  ...
  listDiv.innerHTML = products
    .map(
      (product) => `
                ...
                <p class="tourName">${product.name}</p>
                <p class="tourId">
...
layData("http://localhost:3000/products?isfeatured=true").then(products => renderSanPham(products, "tour-noi-bat"));
layData("http://localhost:3000/products?categoryid=1").then(products => renderSanPham(products, "tour-trong-nuoc"));
layData("http://localhost:3000/products?categoryid=2").then(products => renderSanPham(products, "tour-ngoai-nuoc"));